...
class SmartDisplayApp:
    def __init__(self):
        ...
        self.running = True
        self.net_thread = threading.Thread(target=self._start_network_loop, daemon=True)
            ...
            with urllib.request.urlopen(req, timeout=5) as resp:
                if resp.status == 200:
                    ...
                    if data.get("success"):
                        ...
                        self.weather_dirty = True
                        print(f"[WEATHER] Updated: {data.get('current', {}).get('temp')}°F, {data.get('current', {}).get('condition')}")
    ...
    async def _websocket_listener(self):
        ...
        import websockets
        while self.running:
            if not self.weather_data or time.time() - self.last_weather_fetch > 900:
                self._fetch_weather()

            try:
                print(f"[WS] Connecting to {WS_DISPLAY_URL}...")
                async with websockets.connect(WS_DISPLAY_URL, ping_interval=20, ping_timeout=10) as ws:
                    print("[WS] Connected to dashboard event stream!")
                    while self.running:
                        msg = await ws.recv()
                        data = json.loads(msg)
                        ...
                        if msg_type == "state_change":
                            new_state = payload.get("state", "ready")
                            self.assistant_state = new_state
                            if new_state in ("listening", "thinking", "speaking"):
                                self.last_speech_time = time.time()

                        elif msg_type == "transcription":
                            self.transcription = payload.get("text", "")
                            self.last_speech_time = time.time()

                        elif msg_type == "assistant_reply":
                            self.assistant_reply = payload.get("text", "")
                            self.last_speech_time = time.time()
                ...
                print(f"[WS] Connection dropped ({e}). Retrying in 3s...")
    ...
    def draw_card(self, target_surf, rect, bg=CARD_BG, border=CARD_BORDER, radius=20):
        ...
        pygame.draw.rect(target_surf, border, rect, width=int(1.5 * self.scale), border_radius=r)
    ...
    def draw_clock_section(self):
        ...
        if date_str != self.cached_date_str:
    ...
    def _render_weather_card_surface(self):
        ...
        if not self.weather_data:
            loading_surf = self.font_body.render("Loading weather...", True, TEXT_MUTED)
        ...
        # Condition
        cond_surf = self.font_large.render(condition, True, ACCENT_CYAN)
        ...
        pygame.draw.line(self.weather_card_surf, CARD_BORDER, (int(25 * self.scale_x), detail_y), (self.cw - int(25 * self.scale_x), detail_y), int(1 * self.scale))
    ...
    def _render_forecast_card_surface(self):
        ...
        pygame.draw.line(self.forecast_card_surf, CARD_BORDER, (int(25 * self.scale_x), line_y), (self.rw - int(25 * self.scale_x), line_y), int(1 * self.scale))
        ...
        for day in daily[:6]:
            d_name = day.get("day_name", day.get("short_name", "Day"))
            d_high = day.get("high", "--")
            d_low = day.get("low", "--")
            d_cond = day.get("condition", "Clear")
            d_rain = day.get("rain_prob", 0)

            day_surf = self.font_body.render(d_name, True, TEXT_PRIMARY)
            self.forecast_card_surf.blit(day_surf, (int(30 * self.scale_x), row_y + int(6 * self.scale_y)))

            cond_surf = self.font_small.render(d_cond, True, TEXT_SECONDARY)
            self.forecast_card_surf.blit(cond_surf, (int(140 * self.scale_x), row_y + int(8 * self.scale_y)))

            if d_rain > 0:
                rain_surf = self.font_small.render(f"💧{d_rain}%", True, ACCENT_BLUE)
                self.forecast_card_surf.blit(rain_surf, (self.rw - int(170 * self.scale_x), row_y + int(8 * self.scale_y)))

            hl_surf = self.font_body.render(f"{d_high}° / {d_low}°", True, TEXT_PRIMARY)
            self.forecast_card_surf.blit(hl_surf, (self.rw - hl_surf.get_width() - int(25 * self.scale_x), row_y + int(6 * self.scale_y)))
    ...
    def draw_jarvis_orb(self):
        ...
        base_r = int((52 + pulse) * self.scale)

        # 1. Outer Glow Aura (re-using single pre-allocated aura surface)
        aura_r = min(int(base_r * 1.6), self.max_aura_r - 2)
        self.aura_surf.fill((0, 0, 0, 0))
        pygame.draw.circle(self.aura_surf, (*state_color, 40), (self.max_aura_r, self.max_aura_r), aura_r)
        self.screen.blit(self.aura_surf, (center_x - self.max_aura_r, center_y - self.max_aura_r))

        # 2. Outer Ring
        pygame.draw.circle(self.screen, (*state_color, 180), (center_x, center_y), base_r + int(14 * self.scale), int(2 * self.scale))

        # 3. Inner Core
        pygame.draw.circle(self.screen, state_color, (center_x, center_y), base_r)

        # 4. Highlight Center Core
        pygame.draw.circle(self.screen, (255, 255, 255), (center_x, center_y), int(base_r * 0.35))
    ...
    def draw_right_panel(self):
        ...
        is_speech_active = (time.time() - self.last_speech_time < 12.0) and (self.transcription or self.assistant_reply)
        ...
        if is_speech_active:
            ...
            pygame.draw.line(self.screen, CARD_BORDER, (self.rx + int(25 * self.scale_x), line_y), (self.rx + self.rw - int(25 * self.scale_x), line_y), int(1 * self.scale))
    ...
    def _render_wrapped_text(self, text, x, y, max_width, font, color):
        """Simple word-wrap renderer for dialogue text."""
        words = text.split(" ")
        lines = []
        curr_line = []
        ...
        for line in lines:
            ...
            y += s.get_height() + int(4 * self.scale_y)
        return y

    def run(self):
        """Main rendering loop clocked at 30 FPS."""
        print("[DISPLAY] Ultra-efficient display loop started at 30 FPS...")
        while self.running:
